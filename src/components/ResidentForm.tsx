...
export default function ResidentForm({ initial, onSave, title, subtitle }: ResidentFormProps) {
  const { register, handleSubmit, formState: { errors }, control } = useForm<ResidentFormInputs>({ defaultValues: initial });
  ...
  <div className="w-full mx-auto px-2 max-w-sm">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div className="flex flex-col gap-2">
          <Label className="text-xs flex items-center gap-1 mb-0 font-semibold text-blue-900"><Home className="w-4 h-4" />Blok & Nomor Rumah</Label>
        ...
        <div>
          <Label htmlFor="name" className="mb-1 text-xs font-semibold text-blue-900 flex items-center gap-1">
            <User className="w-4 h-4" />Nama Warga
          </Label>
        ...
        <div>
          <Label htmlFor="phoneNumber" className="mb-1 text-xs font-semibold text-blue-900 flex items-center gap-1">
            <Phone className="w-4 h-4" />Nomor Telepon <span className="text-xs text-blue-500">(opsional)</span>
          </Label>
