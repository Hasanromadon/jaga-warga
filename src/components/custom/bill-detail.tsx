  ...
  return (
    <div className="rounded-2xl bg-white/90 border border-blue-100  p-5 flex flex-col gap-3 transition-all duration-200 hover:shadow-blue-200/60 animate-fade-in">
      ...
      <div className="flex items-center gap-3">
        <div className="rounded-full bg-blue-100 p-2 flex items-center justify-center self-start mt-1">
          <Home className="w-5 h-5 text-blue-600" />
        </div>
        <div className="flex-1 min-w-0 flex flex-col justify-center">
          <div className="font-bold text-blue-900 text-lg leading-snug break-words whitespace-pre-line mb-0.5">
      ...
      <div className="flex flex-col items-center gap-0.5">
        ...
        <div className="text-blue-700 font-extrabold text-2xl tabular-nums leading-tight tracking-tight text-center">
      ...
      <div className="flex flex-row items-center justify-between gap-2">
        <div className="flex flex-col">
          ...
          <span className="text-blue-700 font-semibold text-sm">
            {
              BULAN_LIST.find(
                (b: { value: string; label: string }) => b.value === bill.month,
              )?.label
            }
            /{bill.year}
          </span>
        ...
        <div className="my-2 flex flex-col gap-1.5">
          {bill.status === 'rejected' && (
            <div className="bg-red-50  border border-red-200 text-red-700 rounded px-3 py-2 text-xs mb-4">
              ...
              {bill.rejectReason ? (
                <div>Alasan: {bill.rejectReason}</div>
              ) : (
                <div>Tidak ada alasan penolakan.</div>
              )}
            </div>
          )}
          <label className="font-semibold text-xs mb-1">
            Unggah {bill.status === 'rejected' ? 'Ulang ' : ''}Bukti Pembayaran
          </label>
          <Input
            type="file"
            ...
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
              ...
              if (file) {
                ...
                if (!allowedTypes.includes(file.type)) {
                  toast.error(
                    'File harus berupa gambar (jpg, png, webp, gif) atau PDF',
                  );
                  e.target.value = '';
                  setBukti(null);
                  return;
          ...
          <div className="text-[11px] text-blue-500 mt-1">
            Hanya menerima file gambar (JPG, PNG, WEBP, GIF) atau PDF. Ukuran
            maksimal 3MB.
          </div>
          <Button
            onClick={handleUpload}
            disabled={uploading || !bukti}
            className="w-full flex items-center justify-center gap-2 text-xs h-9 mt-1"
          >
            {uploading ? (
