          ...
          <Card key={bill.id} className="animate-fade-in border border-gray-200 bg-white/95 shadow-sm rounded-xl">
            <div className="px-4 pt-3 pb-2">
              <div className="flex flex-wrap gap-2 items-center w-full mb-4">
                ...
                <span className="ml-auto text-xs text-gray-400 font-normal">{formatTimestampID(bill.createdAt)}</span>
              </div>
              <div className="font-semibold text-blue-900 text-sm truncate mb-1" title={`Blok ${bill.block} No ${bill.houseNumber}`}>{`Blok ${bill.block} No ${bill.houseNumber}`}</div>
          ...
          return (
            <div className="text-left space-y-2">
              <div className="grid grid-cols-[120px_1fr] gap-x-4 gap-y-1 text-sm items-center">
                <span className="text-gray-500">Blok/No</span> <span>{bill.block}/{bill.houseNumber}</span>
                <span className="text-gray-500">Month/Year</span> <span>{bill.month}/{bill.year}</span>
                <span className="text-gray-500">Created At</span> <span>{formatTimestampID(bill.createdAt)}</span>
