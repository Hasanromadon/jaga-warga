    ...
    <Card className="max-w-sm w-full mx-auto">
      ...
      <CardContent>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
          ...
          <Input type="password" placeholder="Password" {...register('password', { required: true })} autoComplete="current-password" className="text-sm" />
          {errors.password && <span className="text-red-500 text-xs">Password wajib diisi</span>}
          <Button type="submit" disabled={loading} className="w-full text-base">{loading ? 'Loading...' : 'Login'}</Button>
        </form>
